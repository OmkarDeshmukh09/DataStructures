...
void DeleteLast(PPNODE first)
{
    ...
    {
        temp = *first ;
        while (temp ->next ->next != NULL)      //Type 3 Loop
        {
            temp = temp -> next;
        }
        free(temp->next);
...
{    
    ...
    else
    {
      ...
      target = temp -> next;        // number 1
      temp->next = target->next;    //number 2
